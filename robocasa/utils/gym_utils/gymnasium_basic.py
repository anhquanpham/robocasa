        ...
        env_kwargs.update(
            {
                "layout_ids": None,
                "style_ids": None,
                # Match robocasa_omni: [4, 4] layout; fixed robot position (no spawn deviation)
                "layout_and_style_ids": [[4, 4]],
                "obj_instance_split": "target",  # omni "B" = official "target" (eval split)
                "generative_textures": None,
                "randomize_cameras": False,
                "robot_spawn_deviation_pos_x": 0,
                "robot_spawn_deviation_pos_y": 0,
                "robot_spawn_deviation_rot": 0,
            }
        )
...
class RoboCasaEnv(gym.Env):
    def __init__(
        ...
        **kwargs,  # Accept additional kwargs
        ...
        # [gr00t-guided merge] Only for gym_utils (GR00T wrapper). Official robocasa's wrappers/
        # gym_wrapper.py already call reset() before accessing robots. Here, with robocasa365 +
        # robosuite master, robots are not populated until reset(). robocasa_omni did not need this.
        self.env.reset()
    ...
    def step(self, action_dict):
        ...
        # For rollout_policy: mark episode as valid when it ends (placement succeeded, no crash).
        # BEHAVIOR/OmniGibson set valid=False on PhysX crash; robocasa has no such path.
